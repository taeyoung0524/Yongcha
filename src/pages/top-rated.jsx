...
const fetchMovies = async (page) => {
  const response = await fetch(`https://api.themoviedb.org/3/movie/top_rated?language=ko&page=${page}`, {
    headers: {
...
const TopRatedPage = () => {
  ...
  const { data: movies, isLoading, isError } = useQuery({
    queryKey: ['topRatedMovies', page], // 페이지별 데이터 캐시를 위해 queryKey에 페이지 번호 포함
